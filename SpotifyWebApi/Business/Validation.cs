namespace SpotifyWebApi.Business
{
    ...
    internal static class Validation
    {
        ...
        public static void ValidateList<T>(ICollection<T> list, string paramName, int min = -1, int max = -1)
        {
            if (list == null) { throw new ValidationException("The list is null.", paramName); }

            var c = list.Count;
            if (c == 0) throw new ValidationException("The list is empty.", paramName);
            if (c < min) throw new ValidationException("The list is too small.", paramName);
            if (c > max) throw new ValidationException("The list is too big.", paramName);
        }
        ...
        public static void ValidateInteger(int value, string paramName,  int min = int.MinValue, int max = int.MaxValue)
        {
            if (value < min) throw new ValidationException($"The value is too small. Value: {value}, min: {min}", paramName);
            if (value > max) throw new ValidationException($"The value is too big. Value: {value}, max: {max}", paramName);
        }
        ...
        /// <param name="paramName">The name of the parameter.</param>
        /// <exception cref="ValidationException">Throws a validation exception when the value is invalid.</exception>
        public static void ValidateToken(Token token, string paramName)
        {
            if (token == null) { throw new ValidationException("The token is null.", paramName); }

            if (token.Type != "Bearer") throw new ValidationException("The token type is not \"Bearer\".", paramName);

            if (string.IsNullOrWhiteSpace(token.AccessToken))
                throw new ValidationException("The token's 'AccessToken' is empty or null", paramName);
